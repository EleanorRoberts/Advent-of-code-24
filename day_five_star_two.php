...
function addCorrectPageToTotal(int $total, array $update, array $rules): int {
    ...
    foreach ($incorrect as $toFix) {
        $fixedOrder = reorderUpdate($toFix, $rules);
    }
...
function reorderUpdate($update, $rules): array
{
    $sorted = usort($update, function ($a, $b) {
